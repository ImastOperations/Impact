...
public class CostumerAdapter extends RecyclerView.Adapter<CostumerAdapter.RecylerviewViewholder> {
    private final EmployeInterface buttonListener;
    Context context;
    private ArrayList<GetCategory> arrayList;
    private String get_idd;
    private String get_name;

    public CostumerAdapter(Context mcontext, ArrayList<GetCategory> arrayList, EmployeInterface buttonListener) {
        context = mcontext;
    ...
    public void onBindViewHolder(@NonNull RecylerviewViewholder holder, int position) {
        ...
        holder.text_view1.setOnClickListener(v -> {
            ...
            buttonListener.foo1(get_idd, get_name);
            //dialog.dismiss();
        });
    }
